...
###############################################################
def Training():
    print('====== Training On RGB_IPM Datasets ======')
    ...
    print("=== eval label path ===",p.test_root_url + '/data/test_converted.json')
    ##############################
    ## Check GPU
    ##############################
    ...
    for epoch in range(p.n_epoch):
        ...
        if epoch >= 100 and epoch%1 == 0:
            ...
            # lane_agent.save_model(int(step/100), 0.888)
            ...
            for idx in index:
                ...
                with open("./eval_results/eval_result_"+str(idx)+"_.txt", 'a') as make_file:
                    ...
                    # make_file.write( "epoch : " + str(epoch) + " loss : " + str(0.888) )
                    make_file.write(evaluation.LaneEval.bench_one_submit("test_result_"+str(epoch)+"_"+str(idx)+".json", p.test_root_url + '/test_converted.json'))
                    make_file.write("\n")
