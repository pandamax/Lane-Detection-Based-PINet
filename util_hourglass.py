...
class Conv2D_BatchNorm_Relu(nn.Module):
    def __init__(self, in_channels, n_filters, k_size, padding, stride, bias=True, acti=True, dilation=1):
        ...
        if acti:
            ...
            self.cbr_unit =  nn.Sequential(nn.Conv2d(in_channels, n_filters, k_size, padding=padding, stride=stride, bias=bias, dilation=dilation),)
...
class bottleneck_down(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv3 = nn.Sequential(nn.Conv2d(temp_channels, out_channels, 1, padding=0, stride=1, bias=True),)
...
class bottleneck_up(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        if in_channels < 4:
            temp_channels = in_channels
        self.conv1 = nn.Sequential(nn.ConvTranspose2d(in_channels, temp_channels, 2, stride=2, padding=0, output_padding=0, groups=1, bias=True),
                                    )
...
class hourglass_same(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs = self.same3(self.relu(feature))  # 4*2 -> 4*2

        outputs = self.same4(outputs, True)  # 4*2 -> 4*2

        outputs = self.up1( self.relu(outputs + self.residual1(outputs3, True)) )

        outputs = self.up2( self.relu(outputs + self.residual2(outputs2, True)) )
        outputs = self.up3( self.relu(outputs + self.residual3(outputs1, True)))
        outputs = self.up4( self.relu(outputs + self.residual4(inputs, True)) )
...
class resize_layer(nn.Module):
    def __init__(self, in_channels, out_channels, acti = True):
        ...
        self.conv3 = Conv2D_BatchNorm_Relu(out_channels//2, out_channels//1, 3, 1, 2, dilation=1, acti = True) # base:resize_ratio = 8.
...
class hourglass_block(nn.Module):
    def __init__(self, in_channels, out_channels, acti = True, input_re=True):
        ...
        self.re2 =  nn.Sequential(nn.Conv2d(out_channels, out_channels, 1, padding=0, stride=1, bias=True, dilation=1),)
        self.re3 =  nn.Sequential(nn.Conv2d(1, out_channels, 1, padding=0, stride=1, bias=True, dilation=1),)
        self.re4 =  nn.Sequential(nn.Conv2d(out_channels, out_channels, 1, padding=0, stride=1, bias=True, dilation=1),)
    ...
    def forward(self, inputs):
        ...
        # outputs_a_base = self.bn2(outputs)
        outputs_a = outputs

        outputs_a = self.relu(outputs_a)
        ...
        out = self.relu(out_confidence)
        out = self.re3(out)


        if self.input_re:
            # avoid continuously add operation
            out_add_inputs = self.re4(out + inputs)
            outputs = outputs + out_add_inputs
        else:
            outputs = outputs + out
