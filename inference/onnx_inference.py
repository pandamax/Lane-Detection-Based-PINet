...
envs:(yolact) pytorch_version == 1.4
'''
...
def onnx_inference(model_path,test_images,save_test_dir):
    ...
    confidences, offsets, instances = sess.get_outputs()[0].name, sess.get_outputs()[1].name, sess.get_outputs()[2].name
    ...
    for img in img_list:
        ...
        _, _, ti = test(pred_onnx, ori_image, w_ratio, h_ratio, crop_start_h, thresh=p.threshold_point)
...
if __name__ == '__main__':
    ...
    index = 16
    loss = 0.7522
    onnx_model = '../converter/onnx_models/pinet_256x256_{}_{}_sim_edit_rifi.onnx'.format(index, loss) 
    save_test_dir = './inference_test_images/test_onnx_result_{}_{}'.format(index, loss)
    ...
    for test_dateset in test_dataset_list:
        ...
        onnx_inference(onnx_model, test_images, save_test_dir)
